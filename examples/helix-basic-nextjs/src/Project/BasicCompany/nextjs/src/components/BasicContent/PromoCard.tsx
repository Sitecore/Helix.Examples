import { Text, RichText, Link, Field, ImageField, LinkField } from '@sitecore-jss/sitecore-jss-nextjs';
...
const PromoCardWrapper : React.FunctionComponent<PromoCardWrapperProps> = ({linkField, children}) => {
    const wrapperProps = {
        className: "column promo-column"
    };
    return linkField?.value?.href ? 
        <Link field={linkField} editable={false} {...wrapperProps}>{children}</Link>
